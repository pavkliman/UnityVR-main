...
public class ListObjectTransform : MonoBehaviour
{
    ...
    public void TransformObj(float duration)
    {
        foreach (KeyValuePair<GameObject, Vector3> kvp in myDict)
        {
    ...
    public void BackTransformObj(float duration)
    {
        int i = 0;
        foreach (KeyValuePair<GameObject, Vector3> kvp in myDict)
        {
    ...
    private IEnumerator Logic()
    {
        ...
        if (logic)
        {
            TransformObj(duration);
            logic = false; 
        }
        else
        {
            BackTransformObj(duration);
            logic = true;
    ...
    {
        if (other.CompareTag("Player") && logic == false)
        {
            BackTransformObj(1.0f);
            logic = true;
        }
    }
}
